...
class PlateauLRScheduler(Scheduler):
    ...
    def __init__(self,
        ...
        if self.warmup_t:
            # when resume_epoch > self.warmup_t, the lr is always warmup_lr_init
    ...
    def step(self, epoch, metric=None):
        if epoch <= self.warmup_t:
            ...
            # todo: set for cosine/multistep
            for i, param_group in enumerate(self.optimizer.param_groups):
                if 'no_lr_decay' in param_group.keys():
                    if param_group['no_lr_decay']:
                        base_lr = float(self.base_values[i])
                        param_group['lr'] = base_lr
                        name = getattr(param_group,'name',i)
                        print('Not reducing learning rate '
                            'of group {}, retains {:.4e}.'.format(name, base_lr))
